...
export default function GroceryItem({ item, onToggle, onDelete }) {
  ...
  const cat = GROCERY_CATEGORIES.find((c) => c.id === item.category) || GROCERY_CATEGORIES[GROCERY_CATEGORIES.length - 1];
  ...
  return (
    <div className="relative overflow-hidden rounded-xl mb-2">
      ...
      <motion.div
        ...
        className={`relative bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl px-4 py-3 flex items-center gap-3 ${swiped ? '' : ''}`}
      >
        {/* Checkbox */}
        <button
          onClick={onToggle}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className={`text-xs ${cat.colorClass} px-1.5 py-0.5 rounded-full`}>{cat.icon} {cat.label}</span>
          </div>
          {item.name_hi && (
