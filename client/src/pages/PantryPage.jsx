...
const ExpiryBadge = ({ expiryDate }) => {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${colors[status.status]}`}>
      {status.label}
    </span>
  );
};
...
export default function PantryPage() {
  ...
  const handleAdd = async (e) => {
    ...
    setLoading(true);
    try {
      await addItem({ ...form, quantity: parseFloat(form.quantity) || 1, purchase_price: parseFloat(form.purchase_price) || null });
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {item.expiry_date && <ExpiryBadge expiryDate={item.expiry_date} />}
                      </div>
