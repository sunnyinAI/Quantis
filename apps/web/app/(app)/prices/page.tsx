...
export default function PricesPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    (comparePrices(debouncedQuery, parseFloat(qty) || 1, 'kg') as Promise<PriceResult>)
        ...
        <AnimatePresence>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-gray-800 dark:text-gray-100">{result.item} ({qty} kg)</h2>
              {available.length > 0 && (
            ...
            <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {platform.is_best && (
                            <span className="text-xs bg-kgreen-100 dark:bg-kgreen-900/30 text-kgreen-700 dark:text-kgreen-400 px-2 py-0.5 rounded-full font-medium flex items-center gap-1">
              ...
              {unavailable.length > 0 && (
                <p className="text-xs text-gray-400 mt-2">Not available on: {unavailable.map((p) => p.name).join(', ')}</p>
              )}
            </div>
          </motion.div>
        </AnimatePresence>
