...
interface Commodity { id: string; name: string; name_hi: string; }
interface State { code: string; name: string; }
interface MandiResult {
  mandi: string; unit: string; modal_price: number; min_price: number;
  max_price: number; prev_price: number; change_pct: number;
}
interface MandiData {
  commodity?: { name: string };
  insight?: { recommendation: string; mandi_price_per_kg: number; app_delivery_price_per_kg: number; savings_pct: number };
  results?: MandiResult[];
}
...
export default function MandiPage() {
  ...
  useEffect(() => {
    (getCommodities() as Promise<{ commodities: Commodity[]; states: State[] }>)
      .then(({ commodities, states }) => { setCommodities(commodities); setStates(states); })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    (getMandiPrices(selectedCommodity, selectedState) as Promise<MandiData>)
      .then(setData)
      .catch(() => addToast('Failed to fetch mandi prices', 'error'))
      .finally(() => setLoading(false));
  }, [addToast, selectedCommodity, selectedState]);

  const trending = (changePct: number) => {
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-4 mt-2 text-xs">
                    <div><span className="text-gray-500">Mandi:</span> <span className="font-semibold">₹{data.insight.mandi_price_per_kg}/kg</span></div>
                    <div><span className="text-gray-500">Delivery:</span> <span className="font-semibold">₹{data.insight.app_delivery_price_per_kg}/kg</span></div>
                    {data.insight.savings_pct > 0 && <div><span className="text-kgreen-600 font-semibold">{data.insight.savings_pct}% cheaper</span></div>}
                  </div>
                </div>
              </div>
            </div>
