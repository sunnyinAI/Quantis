...
export default function GroceryPage() {
  ...
  useEffect(() => { if (activeListId) fetchItems(activeListId); }, [activeListId, fetchItems]);
  ...
  const grouped = GROCERY_CATEGORIES.reduce<Record<string, typeof GROCERY_CATEGORIES[number] & { items: typeof activeItems }>>((acc, cat) => {
  ...
  const handleShare = async () => {
    ...
    try {
      const { whatsapp_url, share_code } = await shareList(activeListId) as { whatsapp_url: string; share_code: string };
      addToast(`Share code: ${share_code}`, 'success');
      window.open(whatsapp_url, '_blank');
    } catch (err: unknown) {
      addToast(err instanceof Error ? err.message : 'Failed to share', 'error');
    }
  };
  ...
  const toggleCategory = (catId: string) =>
    setCollapsedCats((s) => ({ ...s, [catId]: !s[catId] }));
    ...
    <div className="page-container">
        ...
        <div className="card mb-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => activeListId && resetList(activeListId)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                <RotateCcw size={16} className="text-gray-500" />
              </button>
            </div>
          </div>
          <div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
            <div className="bg-saffron-500 h-2 rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
          </div>
        </div>
      )}
      ...
      {Object.keys(grouped).length === 0 ? (
        <EmptyState
          ...
          action={filter === 'all' ? <Button onClick={() => setShowAddSheet(true)}>Add First Item</Button> : undefined}
        />
      ) : (
        <AnimatePresence>
              ...
              <AnimatePresence>
                {!collapsedCats[cat.id] && cat.items.map((item) => (
                  ...
                  >
                    <GroceryItem
                      item={item}
                      onToggle={() => activeListId && toggleItem(activeListId, item.id)}
                      onDelete={() => activeListId && deleteItem(activeListId, item.id)}
                    />
                  </motion.div>
                ))}
              </AnimatePresence>
      ...
      <AddItemSheet
        ...
        onAdd={(item) => activeListId ? addItem(activeListId, item) : Promise.resolve()}
      />
    </div>
  );
}
