...
import { Plus, Trash2, CheckCircle, TrendingDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
type Tab = 'Overview' | 'Transactions' | 'Bills' | 'Subscriptions';
const TABS: Tab[] = ['Overview', 'Transactions', 'Bills', 'Subscriptions'];

export default function FinancePage() {
  const {
    transactions, bills, subscriptions, summary, currentMonth,
    fetchAll, addTransaction, deleteTransaction, addBill, payBill, deleteBill,
    addSubscription, toggleSubscription, deleteSubscription, setMonth,
  } = useFinanceStore();
  ...
  const budgetPct = (s?.budget_used_pct as number) || 0;
  ...
  return (
    <div className="page-container">
        ...
        <div className="space-y-4">
          <div className="card flex items-center gap-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-xl font-bold text-gray-900 dark:text-gray-100">{formatCurrency(s.budget as number)}</p>
              <div className="flex gap-4 mt-2 text-sm">
                <div>
                  <p className="text-red-500 font-medium flex items-center gap-1"><TrendingDown size={13} />{formatCurrency(s.total_expense as number)}</p>
                  <p className="text-xs text-gray-400">Spent</p>
                </div>
                <div>
                  <p className="text-kgreen-600 font-medium">{formatCurrency(Math.max(0, (s.budget as number) - (s.total_expense as number)))}</p>
          ...
          {(s.by_category as { category: string; total: number }[])?.length > 0 && (
            <div className="card">
              <p className="font-medium text-gray-700 dark:text-gray-300 mb-3">By Category</p>
              <div className="space-y-2">
                {(s.by_category as { category: string; total: number }[]).sort((a, b) => b.total - a.total).map((cat) => {
                  const c = EXPENSE_CATEGORIES.find((e) => e.id === cat.category);
                  const pct = Math.round((cat.total / (s.total_expense as number)) * 100);
                  return (
          ...
          {(s.upcoming_bills as { id: number; name: string; due_date: string; amount: number }[])?.length > 0 && (
            <div className="card">
              <p className="font-medium text-gray-700 dark:text-gray-300 mb-3">Upcoming Bills</p>
              <div className="space-y-2">
                {(s.upcoming_bills as { id: number; name: string; due_date: string; amount: number }[]).map((bill) => {
                  ...
                  return (
                    <div key={bill.id} className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{days !== null && days <= 0 ? 'Overdue!' : `Due in ${days} days`}</p>
                      </div>
                      <span className={`font-semibold ${days !== null && days <= 0 ? 'text-red-500' : days !== null && days <= 3 ? 'text-yellow-500' : 'text-gray-700 dark:text-gray-300'}`}>
          ...
          <Button className="w-full" onClick={() => setShowTxSheet(true)}><Plus size={18} /> Add Expense</Button>
        </div>
      )}
      ...
      {tab === 'Transactions' && (
        <div>
          {(transactions as unknown[]).length === 0 ? (
            ...
            <div className="space-y-2">
              {(transactions as { id: number; category: string; description: string; date: string; payment_method: string; type: string; amount: number }[]).map((tx) => {
                ...
                return (
                  <div key={tx.id} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl px-4 py-3 flex items-center gap-3">
                    ...
                    <button onClick={() => deleteTransaction(tx.id)} className="p-1 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
                  </div>
                );
              })}
            </div>
      ...
      {tab === 'Bills' && (
        <div>
          {(bills as unknown[]).length === 0 ? (
            ...
            <div className="space-y-2">
              {(bills as { id: number; name: string; due_date: string; amount: number; is_paid: boolean }[]).map((bill) => {
                const days = daysUntilDue(bill.due_date);
                return (
                  <div key={bill.id} className={`bg-white dark:bg-gray-800 border rounded-2xl px-4 py-3 flex items-center gap-3 ${bill.is_paid ? 'border-gray-100 dark:border-gray-700 opacity-60' : days !== null && days <= 0 ? 'border-red-200 dark:border-red-800' : days !== null && days <= 3 ? 'border-yellow-200 dark:border-yellow-800' : 'border-gray-100 dark:border-gray-700'}`}>
                    <div className="flex-1">
                      <p className="font-medium text-gray-800 dark:text-gray-100">{bill.name}</p>
                      <p className={`text-xs mt-0.5 ${bill.is_paid ? 'text-kgreen-600' : days !== null && days <= 0 ? 'text-red-500' : days !== null && days <= 3 ? 'text-yellow-500' : 'text-gray-400'}`}>
                        {bill.is_paid ? '✓ Paid' : days !== null && days <= 0 ? 'Overdue!' : `Due in ${days} days · ${formatDate(bill.due_date)}`}
                      </p>
                    </div>
                    ...
                    <button onClick={() => deleteBill(bill.id)} className="p-1 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
                  </div>
                );
              })}
            </div>
      ...
      {tab === 'Subscriptions' && (
        <div>
          {(subscriptions as unknown[]).length === 0 ? (
            ...
            <div className="space-y-3">
              <div className="card mb-2">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {formatCurrency((subscriptions as { is_active: boolean; billing_cycle: string; amount: number }[]).filter(s => s.is_active).reduce((sum, s) => sum + (s.billing_cycle === 'annual' ? s.amount / 12 : s.amount), 0))}
                  <span className="text-sm font-normal text-gray-400">/mo</span>
                </p>
              </div>
              {(subscriptions as { id: number; name: string; billing_cycle: string; amount: number; renewal_date: string; is_active: boolean }[]).map((sub) => (
      ...
      <Sheet isOpen={showBillSheet} onClose={() => setShowBillSheet(false)} title="Add Bill">
        <form onSubmit={handleAddBill} className="space-y-4">
          <Input label="Bill Name *" placeholder="e.g. Electricity, Gas..." value={billForm.name} onChange={(e) => setBillForm({ ...billForm, name: e.target.value })} />
